...
ANDROID_NDK_ROOT="/Users/ares.li/Android/ndk"
_ANDROID_NDK="/Users/ares.li/Android/ndk"
...
if [[ $? == 0 ]]; then
echo "Start Build OpensSSL..."
OutputDir="$(pwd)/Output/${ARCH}"
make clean
perl -pi -e 's/install: all install_docs install_sw/install: install_docs install_sw/g' Makefile.org
./config no-shared -D__ANDROID_API__=21 no-asm no-ssl3 no-comp no-hw no-engine no-async
